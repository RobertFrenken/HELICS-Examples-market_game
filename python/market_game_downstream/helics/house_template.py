...
from python.market_game_downstream.core.rules import BatteryAction, action_to_market_load
...
class ActionHouse(House):
    """Simpler base class for players who want to choose battery actions.

    Subclasses implement ``choose_action(...)`` and return:
    - ``BatteryAction.CHARGE`` or ``1`` to charge as much as allowed
    - ``BatteryAction.NEUTRAL`` or ``0`` to follow base demand
    - ``BatteryAction.DISCHARGE`` or ``-1`` to discharge as much as allowed

    The inherited ``compute_demand(...)`` converts that action into the
    market-facing load expected by the original game.
    """
...
class DeltaHouse(House):
    """Simpler base class for players who want to choose battery delta.
    ...
    def compute_demand(
        ...
        delta = self.choose_delta(price, hour, battery_charge, demand, price_history)
        return ensure_valid(demand[hour] + delta, demand[hour], battery_charge)
